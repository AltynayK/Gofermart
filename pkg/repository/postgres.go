...
func CreateTableOrders(db *sqlx.DB) {
	_, err := db.Exec("CREATE TABLE IF NOT EXISTS orders (id serial primary key, number varchar UNIQUE, user_id int, status varchar, accrual int DEFAULT 0, uploaded_at timestamptz NOT NULL, withdrawn int, processed_at timestamptz)")
	if err != nil {
		fmt.Print(err)
	}
